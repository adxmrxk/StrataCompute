...
from har_features import apply_calibration, extract_uci_features
...
class ReplayEngine:
    def __init__(self) -> None:
        required = [DEMO_DATA / "activity_replay.csv", DEMO_DATA / "activity_replay_labels.csv",
                    DEMO_DATA / "metadata.json", DEMO_DATA / "feature_calibration.npz",
                    DEMO_DATA / "input_health.npz", MODEL, LABELS]
        ...
        calibration = np.load(DEMO_DATA / "feature_calibration.npz")
        self.gain = calibration["gain"].astype(np.float32)
        self.offset = calibration["offset"].astype(np.float32)
    ...
    def _infer(self, features: np.ndarray) -> dict:
        ...
        self.process.stdin.write(",".join(f"{value:.9g}" for value in features) + "\n")
        ...
        outlier_fraction = float(np.mean((features < self.lower) | (features > self.upper)))
    ...
    def live(self, samples: object) -> dict:
        ...
        features = apply_calibration(raw_features, self.gain, self.offset)
        with self.lock:
            result = self._infer(features)
        visual = self.metadata["visual_features"]
        result.update({
            ...
            "visual_features": [{"name": item["name"], "value": float(features[item["index"]])}
                                for item in visual],
        })
        return result
